...
enum hp_wmi_event_ids {
	...
	HPWMI_HINGE_SENSOR		= 0x24, /* 十进制 36: 铰链传感器 */
};
...
static void hp_wmi_notify(union acpi_object *obj, void *context)
{
	...
	case HPWMI_SMART_EXPERIENCE_APP:
		break;
	case HPWMI_HINGE_SENSOR:
		/* 确保输入设备已经注册了平板模式的 capability */
		if (test_bit(SW_TABLET_MODE, hp_wmi_input_dev->swbit)) {
			/* 当 event_data 为 0x0 时，完全折叠，进入平板模式 (1)
			 * 当 event_data 大于 0x0 时，退出平板模式 (0) */
			// 合上 HPWMI_HINGE_SENSOR: event_data=0x0
			// 笔记本模式 HPWMI_HINGE_SENSOR: event_data=0x4
			// 180度 HPWMI_HINGE_SENSOR: event_data=0x3
			// 往后折 HPWMI_HINGE_SENSOR: event_data=0x2
			// 往后折 HPWMI_HINGE_SENSOR: event_data=0x1
			// 360度 HPWMI_HINGE_SENSOR: event_data=0x0
			if (event_data == 0x0) {
				pr_info("meow, HPWMI_HINGE_SENSOR: event_data=0x%x, SW_TABLET_MODE=enable\n",
					event_data);
			} else {
				pr_info("meow, HPWMI_HINGE_SENSOR: event_data=0x%x, SW_TABLET_MODE=disable\n",
					event_data);
			}
			input_report_switch(hp_wmi_input_dev, SW_TABLET_MODE,
					    event_data == 0x0);
			...
			pr_info("meow, ignore event_id - %d(HPWMI_HINGE_SENSOR) - 0x%x\n", event_id, event_data);
		}
		break;
	default:
...
{
	pr_info("meow from hp_wmi_input_setup\n");
	acpi_status status;
	int err, val;
	char system_device_mode[4] = { 0 };
	const char *chassis_type;
	...
	chassis_type = dmi_get_system_info(DMI_CHASSIS_TYPE);
	if (chassis_type) {
		pr_info("meow, dmi_get_system_info(DMI_CHASSIS_TYPE)=%s\n",
			chassis_type);
	} else {
		pr_info("meow, dmi_get_system_info(DMI_CHASSIS_TYPE) is null\n");
	}
	ret = hp_wmi_perform_query(HPWMI_SYSTEM_DEVICE_MODE, HPWMI_READ,
				   system_device_mode, zero_if_sup(system_device_mode),
				   sizeof(system_device_mode));
	pr_info("meow, hp_wmi_perform_query(HPWMI_SYSTEM_DEVICE_MODE,...) return %d\n", ret);
	if (ret >= 0) {
		pr_info("meow, system_device_mode=[0x%x, 0x%x, 0x%x, 0x%x]\n", system_device_mode[0],system_device_mode[1],system_device_mode[2],system_device_mode[3]);
	}
	/* Tablet mode */
	val = hp_wmi_get_tablet_mode();
	if (!(val < 0)) {
		pr_info("meow, setup SW_TABLET_MODE, hp_wmi_get_tablet_mode=%d\n",
			val);
		__set_bit(SW_TABLET_MODE, hp_wmi_input_dev->swbit);
		input_report_switch(hp_wmi_input_dev, SW_TABLET_MODE, val);
	} else {
		pr_info("meow, force setup SW_TABLET_MODE, hp_wmi_get_tablet_mode=%d\n",
			val);
		__set_bit(SW_TABLET_MODE, hp_wmi_input_dev->swbit);
	}
